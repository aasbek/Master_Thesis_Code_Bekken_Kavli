...
public class PathBuilder_Less_Than_Truckload {
	...
	public PathBuilder_Less_Than_Truckload(Vector<Node> pickupNodes, Vector<Node> deliveryNodes, InstanceData inputdata, PrintWriter pw, Vector<Vehicle> vehicles) {
...
//				System.exit(0);
		...
		if(node.type == "Depot") {
			...
			// If going directly from the start depot to the end depot, return null
			if(L.node.number == 0) {
				return null;
			}
			...
			// Adding all elements from the predecessor's pickup nodes visited to this label's pickup nodes visited
			...
			L2.totalCosts = L.totalCosts + inputdata.fuelPrice*inputdata.fuelConsumptionEmptyTruckPerKm*inputdata.getDistance(L.node,node)
					+ inputdata.fuelPrice*inputdata.fuelConsumptionPerTonKm*L.weightCapacityUsed*inputdata.getDistance(L.node,node)
					+ inputdata.otherDistanceDependentCostsPerKm * inputdata.getDistance(L.node, node)
					+ (inputdata.laborCostperHour + inputdata.otherTimeDependentCostsPerKm)* (L2.time - L.time); 
...
//				System.exit(0);
	...
	public Vector<Label> BuildPaths(Vehicle vehicle, double[] dualVisitedPickupsCon, double[] dualOneVisitCon, BBNode bbNode) {
		...
		//	System.out.println("Remove from unprocessed "+label.toString());
			...
			for(Node pickup:pickupNodes) { 
				// Ensuring that only pickups that are allowed due to the branching matrix are extended
	...
	//	System.out.println("number of dominated labels: "+numberOfDominatedLabels);
		...
		// The labels found list only contains those labels that have positive reduced cost
		return labelsFound;
	}
	...
	public ArrayList<Label> findBestLabel(Vector<Label> labelsFound, BBNode bbNode,  Hashtable<Integer,Label> pathList, Vehicle v) throws NullPointerException {
		...
		// Iterating through the labels found (those with positive reduced cost)
		for(Label i : labelsFound) {
				boolean addLabel = true;
				int pickupNumber = 2;
				// Break if the label does not contain a mandatory pickup node 
				...
				// Add the label to the bestLabelQueue (sorted by highest reduced cost) if it contains all mandatory pickup nodes according to the branching matrix
				if (addLabel == true) {
		...
		while(bestLabels.size()<Math.min(100, bestLabelQueue.size())) {
			...
			// Add path to the label and compute the average capacity utilization on the route
			currentBestLabel.path = new Vector<Node>();
			Label temp = currentBestLabel.predesessor;
			double totalDistance = currentBestLabel.totalDistance;
			while(temp!=null) {
				if(temp.node.number != 0) {
					currentBestLabel.capacityUtilization += (temp.weightCapacityUsed / 30) * (inputdata.getDistance(temp.node, temp.predesessor.node)/totalDistance);
				}
			...
			// Check if the route of the label is already existing for another label
			...
			if(! routeAlreadyExisting) {
				...
				// If not already existing, add route to the list of bestlabels that is returned to the master problem
				if(! routeAlreadyExisting ) {
